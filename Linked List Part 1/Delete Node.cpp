...
void solve() {
    ...
    cin >> pos;  // position of node to delete (0-based, not last)
    ListNode* nodeToDelete = list.getNodeAt(pos);

    if (nodeToDelete == nullptr || nodeToDelete->next == nullptr) {
        cout << "Invalid node for deletion (either null or last node)." << endl;
        return;
    }
